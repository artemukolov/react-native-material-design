...
export default class Section extends Component {
        ...
        return (
            ...
            >
                ...
                <View style={styles.value}>
                    <Text style={[TYPO.paperFontBody2, { color }, {fontWeight: 'normal', fontFamily: 'Roboto'}]}>
                        {item.value}
                    </Text>
                </View>
                {item.label &&
    ...
    render() {
        const { theme, title, items, activeColor, activeFontColor } = this.props;
        ...
        const ACTIVE_TEXT_COLOR = activeFontColor || null;
        const SUB_TEXT_COLOR = subheaderStyleMap[theme];
        const ACTIVE_COLOR = activeColor || activeStyleMap[theme];
                ...
                {items && items.map((item, i) => {
                    ...
                    if (!isCompatible('TouchableNativeFeedback')) {
                        return (
                            ...
                            >
                                {this.renderRow(item, i, item.active ? ACTIVE_TEXT_COLOR : TEXT_COLOR)}
                            </Ripple>
                        );
                    }
